...
const ENTITY_CONFIG = {
  venues: {
    ...
    completenessFields: ["name", "slug", "category", "short_description", "long_description", "area", "destination", "image_url", "ai_summary"],
  },
  places: {
    entityType: "place",
    table: "places",
    ...
    imageColumn: "image_url",
    publishedFilter: "status = 'published' AND editorial_status <> 'hidden'",
    completenessFields: ["name", "slug", "category", "description", "area", "destination", "latitude", "longitude", "image_url", "ai_summary"],
  },
  experiences: {
    ...
    imageColumn: "image_url",
    publishedFilter: "status = 'published' AND editorial_status <> 'hidden'",
    completenessFields: ["name", "slug", "category", "description", "area", "destination", "image_url", "ai_summary"],
  },
  articles: {
    entityType: "article",
    table: "articles",
    ...
    publishedFilter: "(status = 'published' OR published = TRUE) AND editorial_status <> 'hidden'",
    completenessFields: ["title", "slug", "category", "excerpt", "featured_image", "author", "published_at", "ai_summary"],
  },
  events: {
    entityType: "event",
    table: "events",
    ...
    imageColumn: "image_url",
    publishedFilter: "status = 'published' AND editorial_status <> 'hidden'",
    completenessFields: ["title", "slug", "category", "description", "destination", "area", "start_datetime", "image_url", "ai_summary"],
  },
};
...
function buildEntityQuery({ config, filters, limitParam, entityTypeParam, scoreExpression = null }) {
  const scoreSelect = scoreExpression ? `, ${scoreExpression} AS recommendation_score` : "";
...
async function queryEntityList({ entityType, filters, values, limit, scoreExpression = null }) {
  ...
  const result = await getPool().query(
    buildEntityQuery({ config, filters, limitParam, entityTypeParam, scoreExpression }),
    values,
  );
...
export async function searchDestinationContent({
  ...
  const resultSets = await Promise.all(
    entityTypes.map((entityType) => {
      ...
      return queryEntityList({ entityType, filters, values, limit: perEntityLimit });
    }),
  );
...
export async function listRecommendations({
  ...
  return resultSets
    .flat()
    .sort((left, right) => (right.recommendationScore ?? right.priorityScore) - (left.recommendationScore ?? left.priorityScore))
    .slice(0, normalizeLimit(limit, 20));
}

export async function listFeaturedContent(destination, entityType = "venues", limit = 12) {
...
export async function listRecommendedContent(destination, entityType = "venues", limit = 12) {
  const entityKey = getEntityKey(entityType);
  const config = getEntityConfig(entityKey);
  const values = [];
  const filters = [];
