...
export function buildPromoPassId(stripeSessionId) {
  ...
  const numericDigest = BigInt(`0x${digest || "0"}`);
  const base36 = numericDigest.toString(36).toUpperCase();

  return base36.padStart(8, "0").slice(0, 8);
}

function buildPasskitPassId(passId) {
  return buildVerifyUrl(passId);
}

function buildVerifyUrl(passId) {
  return `https://ahangama.com/verify?${encodeURIComponent(passId || "")}`;
}
...
export async function syncPromoPurchaseFromSession(session, options = {}) {
  ...
  const passUrl = buildPromoPassUrl(passId);
