...
function normalizeExternalUrl(value, { defaultHost } = {}) {
  ...
  if (url.startsWith("@") && defaultHost) {
    return `https://${defaultHost}/${url.slice(1)}`;
  }
...
function mapEventRow(row) {
  ...
  return {
    ...
    instagramUrl: normalizeExternalUrl(row.instagram_url, {
      defaultHost: "www.instagram.com",
    }),
    directionsUrl: normalizeExternalUrl(row.directions_url || row.venue_google_url),
    dayKey,
    date: "",
  };
}
