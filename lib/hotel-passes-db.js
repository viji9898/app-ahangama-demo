...
export async function createPassGuest(input = {}) {
  ...
  const sourceHotelSlug = normalizeHotelSlug(input.sourceHotelSlug || input.sourceHotel);
  ...
  if (
    ...
    const existingResult = await getPool().query(
      `
        ...
        RETURNING *
      `,
      [normalizedGuestId, sourceHotelSlug, input.validFrom || null, input.validUntil || null],
    );
...
export async function createGuestPreferences(input = {}) {
  const result = await getPool().query(
    ...
    [
      ...
      normalizeTextArray(
        input.bookingInterests ?? input.servicesInterestedIn ?? input.servicesInterested,
      ),
      normalizeBoolean(
...
export async function updateGuestPreferences(passId, updates = {}) {
  ...
  if (
    ...
    Object.prototype.hasOwnProperty.call(updates, "wantsWhatsappRecommendations")
  ) {
    values.push(
      normalizeBoolean(
    ...
    if (
      ...
      passResult = await client.query(
        `
          ...
          RETURNING *
        `,
        [guest.id, passSourceHotelSlug, input.validFrom || null, input.validUntil || null],
      );
    }
