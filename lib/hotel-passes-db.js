...
function mapGuestPreferencesRow(row) {
  ...
  const servicesInterestedIn = row.services_interested || [];
  ...
  return {
    ...
    servicesInterested: servicesInterestedIn,
    whatsappOptIn,
...
export async function createGuestPreferences(input = {}) {
  const result = await getPool().query(
    ...
    [
      ...
      normalizeTextArray(
        input.servicesInterestedIn ?? input.servicesInterested,
      ),
      normalizeBoolean(
...
export async function updateGuestPreferences(passId, updates = {}) {
  ...
  if (
    ...
    values.push(
      normalizeTextArray(
        updates.servicesInterestedIn ?? updates.servicesInterested,
      ),
    );
