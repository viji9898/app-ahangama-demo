...
import pg from "pg";
import { buildPromoPassUrl } from "./promo-urls.js";
...
function normalizeRedemptionPrefixComparable(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/(?:^|\s|-)(ag|ahangama)$/i, "")
    .replace(/[^a-z0-9]+/g, "");
}

function buildRedemptionPrefix(venueSlug, venueName) {
  const slugPrefix = String(venueSlug || "")
    .toLowerCase()
    .replace(/(?:-ag|-ahangama)$/i, "")
    .replace(/[^a-z0-9]+/g, "");
  const namePrefix = String(venueName || "")
    .toLowerCase()
    .replace(/(?:\s+ag|\s+ahangama)$/i, "")
    .replace(/[^a-z0-9]+/g, "");
  const comparableSlug = normalizeRedemptionPrefixComparable(venueSlug);
  const comparableName = normalizeRedemptionPrefixComparable(venueName);

  if (namePrefix && comparableName && comparableName !== comparableSlug) {
    return namePrefix;
  }

  return slugPrefix || namePrefix || "redemption";
}

function buildRedemptionNumber(prefix, sequence) {
  return `${prefix || "redemption"}${String(sequence).padStart(4, "0")}`;
}
...
function mapPromoPurchaseRow(row) {
  ...
  return {
    ...
    passId,
    passUrl: passId ? buildPromoPassUrl(passId) : row.pass_url,
...
export async function getLatestPromoPurchaseByCustomerEmail(customerEmail) {
  const result = await getPool().query(
    `
      SELECT *
      FROM ${PROMO_PURCHASES_TABLE}
...
export async function createPromoRedemption(input = {}) {
  ...
  const venueSlug = normalizeText(input.venueSlug);
  const venueName = normalizeText(input.venueName);

  try {
    await client.query("BEGIN");
    await client.query(
      "SELECT pg_advisory_xact_lock(hashtext($1))",
      [venueSlug || passId || "promo-redemption"],
    );
    ...
    if (existingResult.rows[0]) {
      ...
      return mapPromoRedemptionRow(existingResult.rows[0]);
    }

    const sequenceResult = await client.query(
      `
        SELECT COALESCE(
          MAX((substring(redemption_number FROM '([0-9]+)$'))::integer),
          0
        ) AS current_sequence
        FROM ${PROMO_PASS_REDEMPTIONS_TABLE}
        WHERE venue_slug = $1
          AND redemption_number IS NOT NULL
          AND redemption_number <> ''
      `,
      [venueSlug],
    );

    const nextSequence = Number(sequenceResult.rows[0]?.current_sequence || 0) + 1;
    const redemptionNumber = buildRedemptionNumber(
      buildRedemptionPrefix(venueSlug, venueName),
      nextSequence,
    );
    ...
    const insertResult = await client.query(
      `
        INSERT INTO ${PROMO_PASS_REDEMPTIONS_TABLE} (
          ...
          $1, $2, $3, $4, COALESCE($5, NOW()), $6, $7, $8, NOW()
        )
        RETURNING *
      `,
      [
    ...
    await client.query("COMMIT");
    return mapPromoRedemptionRow(insertResult.rows[0]);
  } catch (error) {
