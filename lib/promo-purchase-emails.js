...
const CUSTOMER_SUPPORT_URL =
  "https://wa.me/94777908790?text=Hi%20Ahangama%20Pass%2C%20Customer%20Care%21";
...
function formatRedemptionType(value) {
...
function renderReceiptValue(label, value) {
  if (!value) {
    return "Not set";
  }
  ...
  return `<a href="${escapeHtml(value)}" style="color: #0f766e; font-weight: 600; text-decoration: underline;">${linkLabel}</a>`;
}
...
function buildIncludedItemsHtml(purchase) {
  ...
  if (nonPassItems.length >= 2) {
    itemEntries.push(`${nonPassItems.slice(0, 2).map((item) => item.label).join(" + ")} at ${venueName}`);
  } else {
    itemEntries.push(
  ...
  const postcardItem = nonPassItems.find((item) => /postcard/i.test(item.label));
        ...
        .map(
          ([label, value]) => `
            <tr>
              ...
              <td style="padding: 10px 12px; border: 1px solid #e5e7eb;">${renderReceiptValue(label, value)}</td>
            </tr>
          `,
        )
        .join("")}
...
export async function sendPromoCustomerEmail(purchase, options = {}) {
  ...
  const venueName = formatVenueName(purchase.venueSlug) || "your selected venue";
  const includedItemsHtml = buildIncludedItemsHtml(purchase);
  const walletButtonHtml = purchase.passkitUrl
    ? `
        <p style="margin: 0 0 12px;">
          <a href="${escapeHtml(purchase.passkitUrl)}" style="display: inline-block; padding: 12px 18px; background: #0f766e; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 700;">Add Pass to Digital Wallet</a>
        </p>
      `
    : "";
  const passButtonHtml = `
      <p style="margin: 0;">
        <a href="${escapeHtml(purchase.passUrl)}" style="display: inline-block; padding: 12px 18px; background: #ffffff; color: #111827; text-decoration: none; border-radius: 8px; font-weight: 700; border: 1px solid #d1d5db;">View Your Digital Pass</a>
      </p>
    `;
  ...
  const html = baseEmailShell(
    ...
    `
      ...
      ${includedItemsHtml}
      <p>
        <a href="${escapeHtml(AHANGAMA_VENUES_MAP_URL)}" style="color: #0f766e; font-weight: 600; text-decoration: underline;">Explore Ahangama Pass venues on Google Maps</a>
      </p>
      ...
      <p style="margin-top: 24px;">
        <a href="${escapeHtml(CUSTOMER_SUPPORT_URL)}" style="display: inline-block; padding: 12px 18px; background: #0f766e; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 700;">Customer Support on WhatsApp</a>
      </p>
    `,
  );
  ...
  const html = baseEmailShell(
    ...
    `
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 24px 0;">
        ...
        <tr>
          <td style="padding: 10px 12px; border: 1px solid #e5e7eb; width: 160px; font-weight: 700; background: #f9fafb; vertical-align: top;">Offer Redeemed</td>
          <td style="padding: 10px 12px; border: 1px solid #e5e7eb;">${escapeHtml(redemption?.offerUsed || purchase.productDescription || purchase.productName || "Promo offer")}</td>
        </tr>
        <tr>
          ...
          <td style="padding: 10px 12px; border: 1px solid #e5e7eb;">${escapeHtml(formatRedemptionType(redemption?.redemptionType))}</td>
        </tr>
        <tr>
