...
export async function sendPromoCustomerEmail(purchase, options = {}) {
  ...
  const walletButtonHtml = purchase.passkitUrl
    ? `<a href="${escapeHtml(purchase.passkitUrl)}" style="display: inline-block; padding: 12px 18px; background: #0f766e; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 700; margin-left: 12px;">Add Pass to Digital Wallet</a>`
    : "";
  ...
  const html = baseEmailShell(
    ...
    `
      ...
      <p>
        <a href="${escapeHtml(purchase.passUrl)}" style="display: inline-block; padding: 12px 18px; background: #111827; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 700;">View Your Digital Pass</a>
        ${walletButtonHtml}
      </p>
