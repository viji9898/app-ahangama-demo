...
const VENUE_NOTIFICATION_EMAIL = "hello@viji.com";
...
function purchaseSummaryHtml(purchase, includeCustomer = false) {
  const rows = [
    ...
    ["Description", purchase.productDescription || "Ahangama Pass promo bundle"],
...
export async function sendPromoVenueEmail(purchase, venue) {
  ensureSendgrid();

  const venueName = venue?.name || purchase.venueSlug || "Partner venue";
  const html = baseEmailShell(
    `New ${escapeHtml(venueName)} promo purchase`,
    `
      <p>A new Ahangama Pass promo purchase has been completed from your QR promo flow.</p>
      ${purchaseSummaryHtml(purchase, true)}
      <p>This email is for visibility only. Redemption continues through the customer’s digital pass.</p>
    `,
  );

  await sgMail.send({
    to: VENUE_NOTIFICATION_EMAIL,
    from: FROM_EMAIL,
    subject: `${venueName} promo purchase notification`,
    html,
  });
}
