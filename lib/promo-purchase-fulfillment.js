...
export async function sendPromoNotifications(purchase) {
  ...
  if (existingPurchase.venueEmailStatus !== "sent") {
    if (!venue?.email) {
      nextStatuses.venueEmailStatus = "skipped";
    } else {
      try {
        await sendPromoVenueEmail(existingPurchase, venue);
        nextStatuses.venueEmailStatus = "sent";
        nextStatuses.venueEmailSentAt = sentAt;
      } catch (error) {
        console.error("Failed to send promo venue email:", error);
        nextStatuses.venueEmailStatus = "failed";
        hasFailure = true;
      }
    }
  }
