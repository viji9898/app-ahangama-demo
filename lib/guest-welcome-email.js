...
const WELCOME_EVENT_LIMIT = 3;

const SOURCE_VENUE_NAMES = {
  "lighthouse-hotel": "Lighthouse",
  "ahangama-comp-pass": "Ahangama Comp Pass",
  "ahangama-hospo": "Ahangama Hospo",
  kaffi: "Kaffi",
  gusta: "Gusta",
...
function getWelcomeEvents(date = new Date()) {
  ...
  const datedEvents = eventSource
    .filter((event) => event.dateKey !== "ongoing")
    .sort((firstEvent, secondEvent) =>
      firstEvent.dateKey.localeCompare(secondEvent.dateKey),
    );
  const upcomingEvents = datedEvents.filter((event) => event.dateKey >= todayKey);
  const latestPastEvents = datedEvents
    .filter((event) => event.dateKey < todayKey)
    .sort((firstEvent, secondEvent) =>
      secondEvent.dateKey.localeCompare(firstEvent.dateKey),
    );
  ...
  for (const event of [...upcomingEvents, ...latestPastEvents, ...ongoingEvents]) {
    if (selectedEvents.length >= WELCOME_EVENT_LIMIT) {
      break;
    }
...
function getWelcomeIntro({ guestName, pass } = {}) {
  if (["ahangama-hospo", "ahangama-comp-pass"].includes(pass?.sourceHotelSlug)) {
...
export function generateGuestWelcomeEmail({ guest, pass } = {}) {
  ...
  const isPublicComplimentaryPass = [
    "ahangama-hospo",
    "ahangama-comp-pass",
  ].includes(pass?.sourceHotelSlug);
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#fff;">
              <tr>
                <td style="padding:24px 20px 24px;background:#fff;border-top:2px solid #111;border-bottom:2px solid #111;color:#111;">
                  ${isPublicComplimentaryPass ? "" : `<div style="font-family:Arial,sans-serif;font-size:9px;line-height:1.2;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:${ACCENT};margin:0 0 10px;">Lighthouse Guest Pass</div>`}
