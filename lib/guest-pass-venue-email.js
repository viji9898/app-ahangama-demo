...
function ensureSendgrid() {
  if (!process.env.SENDGRID_API_KEY) {
    throw new Error("SENDGRID_API_KEY is required for venue notification emails");
  }
...
function buildActionableRecommendations(preferences = {}) {
  ...
  const combined = [...interests, ...services]
    .join(" ")
    .toLowerCase();
...
export function generateGuestPassVenueNotificationEmail({
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-top:2px solid #111;border-bottom:2px solid #111;">
              ...
              <tr>
                <td style="padding:0 22px 22px;">
                  ...
                  ${buildInterestServiceSection(preferences)}
                  <div style="font-family:Arial,sans-serif;font-size:11px;line-height:1.6;color:#333;margin-top:18px;">
                    ${passUrl ? `<a href="${escapeHtml(passUrl)}" style="color:#111;text-decoration:underline;font-weight:700;">Open guest pass</a> &nbsp;` : ""}
                    <a href="${escapeHtml(venuePageUrl)}" style="color:#111;text-decoration:underline;font-weight:700;">Open venue pass page</a>
                  </div>
  ...
  const text = [
    ...
    passUrl ? `Guest Pass: ${passUrl}` : "Guest Pass: Pending",
    `Venue Pass Page: ${venuePageUrl}`,
  ].join("\n");

  return { subject, html, text, recipient: getGuestPassVenueRecipient(resolvedSourceHotelSlug) };
}
