...
function getPool() {
  if (!pool) {
    ...
    pool.on("error", (error) => {
      console.error("Unexpected venues database pool error:", error);
    });
  }
...
function toVenueDto(row) {
  ...
  return {
    ...
    ownership: normalizeOptionalText(row.ownership),
...
export async function listVenues({
  ...
  const sql = `
    SELECT
      ...
      logo, image, og_image,
      map_url, google_place_id, website, ownership, email, instagram, whatsapp,
