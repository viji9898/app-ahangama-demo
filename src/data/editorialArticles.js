export const EDITORIAL_ARTICLES = [
  {
    category: "Things to do",
    title: "Things to Do in Ahangama",
    href: "/things-to-do-in-ahangama",
    description:
      "A practical guide to surf, beaches, food, wellness, shopping, nightlife and carefully chosen south-coast day trips.",
    image:
      "/12 Experiences Images /Why Ahangama Might Be Sri Lanka_s Most Interesting Coastal Town Right Now/The Social side - hero image.webp",
    publishDate: "2026-09-03T09:00:00.000Z",
  },
  {
    category: "Destination Guide",
    title: "Ahangama, Sri Lanka: The Local Guide",
    href: "/ahangama-sri-lanka",
    description:
      "A locally curated introduction to Ahangama's beaches, surf breaks, restaurants, stays, shops and practical travel details.",
    image:
      "/12 Experiences Images /Why Ahangama Might Be Sri Lanka_s Most Interesting Coastal Town Right Now/Hero Image - Opening note.webp",
    publishDate: "2026-09-02T09:00:00.000Z",
  },
  {
    category: "Nightlife Guide",
    title: "Ahangama After Dark",
    href: "/ahangama-after-dark",
    description:
      "Sunset drinks, live music, dinner spots and the changing rhythm of a night out in Ahangama.",
    image: "/Light House Webpage images/light-house-beach-view-hero.webp",
    publishDate: "2026-09-02T09:00:00.000Z",
  },
  {
    category: "Cafe Story",
    title: "Petals Ahangama: A Dream Rooted in Legacy",
    href: "/petals-ahangama-a-dream-rooted-in-legacy",
    description:
      "How a 35-year family legacy in hospitality inspired Sawan Chamika Walgamage to create a welcoming cafe in Kabalana.",
    image:
      "https://res.cloudinary.com/dp7in4ulw/image/upload/w_1200,h_630,c_fill,q_auto,f_auto/v1787205024/Hero_Image_-_Petals-entrance-with-sign_ismfmq.jpg",
    publishDate: "2026-08-18T09:00:00.000Z",
  },
  {
    category: "Shop Guide",
    title: "Gusta: Groceries, Good Food and More",
    href: "/gusta-groceries-good-food-and-more-in-ahangama",
    description:
      "A closer look at Gusta's mix of groceries, specialty food and locally made lifestyle finds in Kabalana.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/gusta-groceries-good-food-and-more-in-ahangama/Hero+Image+-+Outside-ahangama-gusta-outlet.webp",
    publishDate: "2026-08-16T09:00:00.000Z",
  },
  {
    category: "Retreat Guide",
    title: "Host a Retreat in Ahangama",
    href: "/host-a-retreat-ahangama",
    description:
      "Plan a yoga retreat, leadership offsite, creative gathering or fitness camp with trusted local venue and concierge support.",
    image:
      "https://images.openai.com/static-rsc-4/dH7shEGk1uXluF2Jir2c0DGDyYL3Z1kIo7mtn_Fdm219IbR2HYzMwWZJYukJ89v_3Ndbo64q64fWu883O89hVuDk9nHpEKl8N9MnPcbePkvJsC0tOD_JN_5upPYy1qJeOVhQOa9CHEJ67Xjjr1h7N200gaHbZs10vAIJSwWklPk?purpose=inline",
    publishDate: "2026-07-30T09:00:00.000Z",
  },
  {
    category: "Stay Guide",
    title: "Best Stays in Ahangama",
    href: "/stays",
    description:
      "A hand-picked guide to Ahangama's best boutique hotels, beachfront villas and hidden retreats, with locally managed direct-booking enquiries.",
    image:
      "https://images.suitcasemag.com/wp-content/uploads/2025/03/05163113/HERO2-TheFind-SouthCoastSriLanka.jpeg",
    publishDate: "2026-07-27T09:00:00.000Z",
  },
  {
    category: "Stay Guide",
    title: "Best Airbnbs & Private Villas in Ahangama",
    href: "/best-airbnbs",
    description:
      "A hand-picked editorial guide to Ahangama's best private villas, beach houses and hidden retreats for couples, families and groups.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-demo/pebble_alma.jpg",
    publishDate: "2026-07-27T09:00:00.000Z",
  },
  {
    category: "Community",
    title: "Inside the Launch of Ahangama Circle",
    href: "/inside-the-launch-of-ahangama-circle",
    description:
      "Inside the first gathering of south-coast business owners, creatives and hospitality people at Surf Club Midigama.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/inside-the-launch-of-ahangama-circle/hero+-+Guests+networking+at+Surf+Club+Midigama+during+sunset+-+Option+01+.jpeg",
    publishDate: "2026-07-26T09:00:00.000Z",
  },
  {
    category: "Staff Pick",
    title: "A Day That Slowly Erases Your Plan in Ahangama",
    href: "/staff-pick-experience-a-day-that-slowly-erases-your-plan-in-ahangama",
    description:
      "A staff-picked day moving through coffee, surf, cooking, tuk-tuks, sunset and live music.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/staff-pick-experience-a-day-that-slowly-erases-your-plan-in-ahangama/Hero+image+_+ahangama-morning-coffee-hands-cups-close-up.webp",
    publishDate: "2026-06-30T09:00:00.000Z",
  },
  {
    category: "Sunset",
    title: "Best Sunsets in Ahangama",
    href: "/best-sunsets-in-ahangama",
    description:
      "When to arrive, where to stand and which stretches of coast are worth considering for golden hour.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/best-sunsets/Hero+Image+-+2400+x+1600+px.webp",
    publishDate: "2026-06-30T09:00:00.000Z",
  },
  {
    category: "Community",
    title: "Community Markets in Ahangama",
    href: "/community-market-in-ahangama",
    description:
      "Local makers, workshops, music and the pop-up markets that bring Ahangama's community together.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/community-markets-in-ahangama/Hero+image+_+community-markets-ahangama-crowd-stalls.webp",
    publishDate: "2026-06-25T09:00:00.000Z",
  },
  {
    category: "Venue Story",
    title: "Where Ahangama Gathers for Sunset",
    href: "/where-ahangama-gathers-for-sunset-stairway-rooftop-bar-at-lighthouse-hotel",
    description:
      "A sunset story from Stairway Rooftop Bar, where cocktails, ocean views and Ahangama's evening rhythm meet.",
    image:
      "https://res.cloudinary.com/dp7in4ulw/image/upload/v1782832527/light-house-beach-view-hero_mzhrhn.webp",
    publishDate: "2026-06-21T09:00:00.000Z",
  },
  {
    category: "Personal Story",
    title: "Dulasiri Uncle",
    href: "/dulasiri-uncle",
    description:
      "Federica Lazza's quiet personal story about friendship, family warmth and gratitude in Ahangama.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/dulasiri-uncle/Dulasiri-on-the-beach-holding-a-turtle.jpg",
    publishDate: "2026-06-13T09:00:00.000Z",
  },
  {
    category: "Editorial",
    title: "Sri Lanka's Most Interesting Coastal Town",
    href: "/sri-lankas-most-interesting-coastal-town",
    description:
      "Why Ahangama has become a distinctive coastal destination shaped by surf, hospitality, food, wellness and community.",
    image:
      "https://content.r9cdn.net/rimg/dimg/09/d4/c553223f-city-304822-172c638b4d6.jpg?crop=true&width=1366&height=768&xhint=1254&yhint=1207",
    publishDate: "2026-06-12T11:05:00.000Z",
  },
  {
    category: "Transport Guide",
    title: "Getting Around Ahangama",
    href: "/getting-around-ahangama-scooters-tuk-tuks-airport-transfers",
    description:
      "Practical notes on scooters, tuk-tuks, airport transfers and moving through Sri Lanka's south coast.",
    image:
      "https://images.suitcasemag.com/wp-content/uploads/2025/03/21082617/SurfTrip_2042-copy-2.jpeg",
    publishDate: "2026-06-01T12:25:00.000Z",
  },
  {
    category: "Personal Story",
    title: "3 Days in Ahangama",
    href: "/3-days-in-ahangama",
    description:
      "A personal three-day itinerary built around a stay at Samba, coworking blocks, wellness sessions and local perks.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-demo/stays/Asset+33samba.webp",
    publishDate: "2026-05-30T15:45:00.000Z",
  },
  {
    category: "Destination Guide",
    title: "12 Ways to Experience Ahangama",
    href: "/12-things",
    description:
      "An editorial guide to surf, cafes, wellness, creative community, inland rituals and the routines that define daily life.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-demo/ahangama-train-station-sri-Lanka.webp",
    publishDate: "2026-05-27T08:40:00.000Z",
  },
  {
    category: "Stay Guide",
    title: "Where to Stay on Sri Lanka's Southern Coast",
    href: "/where-to-stay-on-sri-lankas-southern-coast",
    description:
      "A guide to the hotels, villas and retreats shaping a new chapter on Sri Lanka's south coast.",
    image:
      "https://images.suitcasemag.com/wp-content/uploads/2025/03/05163113/HERO2-TheFind-SouthCoastSriLanka.jpeg",
    publishDate: "2026-05-22T14:10:00.000Z",
  },
  {
    category: "Shop Guide",
    title: "The Living Room Concept Store",
    href: "/the-living-room-concept-store",
    description:
      "A design-led retail and coffee space shaped around slow discovery and a more thoughtful way to spend time in town.",
    image:
      "https://customer-apps-techhq.s3.eu-west-2.amazonaws.com/app-ahangama-edits/the-living-room-concept-store/hero-the-living-room-concept-store.jpeg",
    publishDate: "2026-05-18T09:15:00.000Z",
  },
  {
    category: "Surf Culture",
    title: "Why Surfing Changed Everything in Ahangama",
    href: "/why-surfing-changed-everything-in-ahangama",
    description:
      "How surf culture reshaped modern Ahangama, from Kabalana mornings to the cafes, camps and creative businesses that followed.",
    image:
      "https://images.pexels.com/photos/19065606/pexels-photo-19065606.jpeg",
    publishDate: "2026-05-11T06:55:00.000Z",
  },
];
